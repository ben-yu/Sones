...
void SpacePhysicsLayer::update(float dt) {
    ...
    CCARRAY_FOREACH( active_lanes,itLane) {
        ...
        if (lane->isVisible() && lane->boundingBox().intersectsRect(playerShip->boundingBox())){
            ...
            dataStoreHandler->saveData(2,(double) searchHelper->cur_freq * seed_freq + 250.0, (double) tmpVol, earIndex, 0,0,0);
            hitFlag = true;
        }
    }
    CCObject *itBoss;
    CCARRAY_FOREACH( _AIEnemies,itBoss) {
        ...
        CCARRAY_FOREACH(_shipLasers,itLaser) {
            ...
            if ( shipLaser->boundingBox().intersectsRect(boss->boundingBox()) ) {
                bossHealth--;
                CCLOG("BOSS HEALTH:  %d",bossHealth);
                bossBar->setPercentage(bossHealth/200.0);
    ...
    CCARRAY_FOREACH( coins,itCoin) {
        ...
        if ( coin->boundingBox().intersectsRect(playerShip->boundingBox()) ) {
            ...
            coin->setPosition(ccp(winWidth, winHeight));
    ...
    if (bossHealth <= 0)
    {
        ...
        CCARRAY_FOREACH( _AIEnemies,itBoss) {
            ...
            patternCount += 2;
            this->scheduleOnce(schedule_selector(SpacePhysicsLayer::spawnAIEnemy), 10.0);
            this->unschedule(schedule_selector(SpacePhysicsLayer::spawnHomingMissle));
            this->unschedule(schedule_selector(SpacePhysicsLayer::spawnAttackPatterns));
        }
    }
...
void SpacePhysicsLayer::deSpawnBeam()
{
    ...
    if (!hitFlag) {
        ...
        CCLog("THRESH: %f",volDiffs[5*earIndex + enemyIndex]);
        dataStoreHandler->saveData(2,(double) searchHelper->cur_freq * seed_freq + 250.0, (double) tmpVol, earIndex, 1,0,0);
    }
}
